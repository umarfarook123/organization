...
exports.validateField = async (fields, reqBody) => {
    ...
    try {
        ...
        let data = fields.map(key => {
            checkFields[key] = key === 'email' ? 'required|email' : key === 'dob' ? 'required|dateFormat:YYYY-MM-DD' : key === 'lastName' ? 'required|maxLength:10' : key === 'firstName' ? 'required|maxLength:10' : key === 'userName' ? 'required|maxLength:10' : key === 'count' ? 'required|numeric|integer|between:1,100' : 'required';
        });
